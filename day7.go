...
func advent7A(test string) (int, error) {
		...
		for loop := 0; programCounters[4] != -1; loop++ {
			...
			for amplifier, phase := range combination {
				if loop == 0 {
					...
					signal, programCounters[amplifier] = softwareB(amplifierSoftware[amplifier], []int{int(phase - '0' + 5), signal}, programCounters[amplifier])
				} else {
					signal, programCounters[amplifier] = softwareB(amplifierSoftware[amplifier], []int{signal}, programCounters[amplifier])
				}
			}
		}
...
func softwareB(instructions []int, input []int, programCounter int) (int, int) {
